# 一键构建 suji 安装包(CPU-only all-in-one)。需先装 Inno Setup 6(ISCC.exe)。
#   powershell -File scripts\build_installer.ps1 [-Iscc <ISCC.exe 路径>] [-QtBin <Qt bin 目录>]
# 步骤: 1) cmake --build Release  2) windeployqt(收集 Qt 运行时到 suji_gui.exe 同目录)  3) ISCC 编译 installer\suji.iss
param(
  [string]$Iscc  = "",
