...
- **安装包 0.6 构建+验证完成**:`build\installer\suji-asr-setup-0.6.exe` **3.7GB**(0.5 是 1.8GB,+1.9GB = fp16 模型 + VC 运行时)。静默装到干净目录 `F:\suji-install-test-06`(5.80GB/102 文件),关键运行时全在(exe / onnxruntime+CUDA provider / sherpa / **msvcp140+vcruntime140[_1]** / Qt+platforms+imageformats / cudart64_12+cudnn / ffmpeg / 两模型+标点)。**最小 PATH(无 dev Qt/CUDA 目录)从安装目录跑 `suji_batch` 通过**:自动 `provider=cuda` + fp16-AED on GPU,`done 1/1 ok`,四格式齐全,转写正确「昨天是MONDAY。」。C1/C2 修复均实证。
